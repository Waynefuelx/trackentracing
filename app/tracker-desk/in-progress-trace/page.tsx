...
export default function InProgressTracePage() {
  const [cases, setCases] = useState<AgentCase[]>(createInitialCases);

  // Filter for in_progress cases only - unsuccessful cases should appear on Unsuccessful Submissions page
    ...
    <section className="space-y-6">
      <header className="space-y-2">
        <h2 className="text-2xl font-semibold tracking-tight text-white">
          In Progress Trace ({inProgressCases.length})
        </h2>
        <p className="text-sm text-slate-300">
          Cases currently being actively traced
        </p>
      </header>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-white">
                Active Traces ({inProgressCases.length})
              </h3>
              <p className="text-xs text-slate-400">
                Cases with ongoing contact attempts
              </p>
            </div>
          </div>
        </header>
              ...
              {inProgressCases.length === 0 ? (
                ...
                inProgressCases.map((caseItem) => (
                  <tr key={caseItem.caseId} className="hover:bg-slate-900/50">
                    ...
                    <td className="whitespace-nowrap px-6 py-3 text-slate-300">
                      {caseItem.lastContactDate || "Not contacted"}
                    </td>
                    ...
                    <td className="whitespace-nowrap px-6 py-3">
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            handleStatusChange(caseItem.caseId, "completed")
                          }
                          className="rounded-xl bg-emerald-500 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
                        >
                          Mark Complete
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            handleStatusChange(caseItem.caseId, "unsuccessful")
                          }
                          className="rounded-xl border border-rose-600/60 bg-rose-600/20 px-3 py-1.5 text-xs font-semibold text-rose-300 transition hover:bg-rose-600/30 focus:outline-none focus:ring-2 focus:ring-rose-400/50"
                        >
                          Mark Failed
                        </button>
                      </div>
